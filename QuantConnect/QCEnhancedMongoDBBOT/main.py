...
# Try to import MongoDB logger with explicit error handling
try:
...
# Dummy MongoDB logger for when MongoDB is not available
class DummyMongoLogger:
    def log_trade(self, *args, **kwargs): pass
    def log_performance_snapshot(self, *args, **kwargs): pass
    def log_signal_evaluation(self, *args, **kwargs): pass
    def flush_all_batches(self): pass
    def log_session_end(self, *args): pass
...
class MayankAlgo_Production(QCAlgorithm):
    def initialize(self) -> None:
        ...
        # Initialize MongoDB logger with import check
        self.debug("=" * 70)
        self.debug("MONGODB LOGGER INITIALIZATION")
        self.debug("=" * 70)
        
        if not MONGODB_AVAILABLE:
            self.debug(f"MongoDB logger import failed: {MONGODB_IMPORT_ERROR}")
            self.debug("MongoDB logging will be disabled")
            self.mongo_logger = DummyMongoLogger()
        else:
            try:
                self.debug("MongoDB logger import successful")
                self.debug("Attempting to initialize MongoDB logger...")
                self.mongo_logger = MongoDBLogger(self, self.logger, self.config)
                self.debug("MongoDB logger object created successfully")
            except Exception as e:
                self.debug(f"Failed to initialize MongoDB logger: {e}")
                self.debug(f"MongoDB logging will be disabled")
                self.mongo_logger = DummyMongoLogger()
        
        self.debug("=" * 70)

        # Test if notify.web() works in backtests
        self.debug("Testing notify.web()...")
        try:
            self.notify.web(
                "https://webhook.site/7b046b65-a22e-4bff-a6e6-0a039760bf3d",  # Replace with your URL
                json.dumps({"test": "backtest webhook", "time": str(self.time)})
            )
            self.debug("Test webhook sent")
        except Exception as e:
            self.debug(f"Test webhook failed: {e}")
        ...
        self.schedule.on(self.date_rules.every_day(self.spy), self.time_rules.at(16, 0), self._log_daily_performance)
        ...
        for symbol in algo_invested:  # Changed from all_invested to algo_invested
            try:
                ...
                if should_exit:
                    ...
                    self.symbol_performance[symbol]['total_pnl'] += pnl_dollar
                    self.symbol_performance[symbol]['win_rate'] = (self.symbol_performance[symbol]['wins'] / self.symbol_performance[symbol]['trades'] * 100) if self.symbol_performance[symbol]['trades'] > 0 else 0
            except Exception as e:
        ...
        if candidates:
            ...
            if qty > 0:
                ...
                if ticket:
                    ...
                    # Log signal that led to entry
                    indicators = self._indicators.get(best_symbol, {})
                    indicator_values = {
                        'macd': indicators['macd'].current.value if 'macd' in indicators else None,
                        'macd_signal': indicators['macd'].signal.current.value if 'macd' in indicators else None,
                        'rsi': indicators['rsi'].current.value if 'rsi' in indicators else None,
                        'ema_50': indicators['ema_50'].current.value if 'ema_50' in indicators else None,
                        'price': current_price
                    }
                    self.mongo_logger.log_signal_evaluation(best_symbol, indicator_values, _, "ENTRY")
    ...
    def on_end_of_algorithm(self) -> None:
        try:
            ...
            final_stats = {
                ...
                'max_drawdown': float((self.peak_equity - final_value) / self.peak_equity if self.peak_equity > 0 else 0)
            }
            self.mongo_logger.log_session_end(final_stats)
            
        except Exception as e:
            self.logger.error(f"End error: {e}")
    
    def on_order_event(self, order_event) -> None:
        """Handle order events and log to MongoDB"""
        ...
        try:
            ...
            # Skip SPY trades (regime tracker only)
            if symbol == self.spy:
                return
            ...
            # Get order tag (reason) from Order object
            order_tag = "unknown"
            try:
            ...
            if is_entry:
                # Log entry trade
                self.mongo_logger.log_trade(
                    symbol=symbol,
                    order_event=order_event,
                    reason="entry"
                )
            else:
                # Log exit trade with P&L
                ...
                self.mongo_logger.log_trade(
                    ...
                    reason=order_tag if order_tag != "unknown" else "exit"
                )
                
        except Exception as e:
            self.logger.error(f"Order event logging error: {e}")
    
    def _log_daily_performance(self) -> None:
        """Log daily performance snapshot to MongoDB"""
        if self.is_warming_up:
            return
        ...
        try:
            ...
            positions = []
            for symbol, holding in self.portfolio.items():
                if holding.invested and symbol != self.spy:
                    current_price = float(self.securities[symbol].price)
                    avg_entry = holding.average_price
                    qty = abs(holding.quantity)
                    pnl = qty * (current_price - avg_entry)
                    ...
                    positions.append({
                        ...
                        'position_type': 'ALGO' if symbol in self._algo_managed_positions else 'MANUAL'
                    })
            
            # Log performance snapshot
            self.mongo_logger.log_performance_snapshot(
                positions=positions,
                market_regime=self.market_regime
            )
            
            # Flush batches at end of day
            self.mongo_logger.flush_all_batches()

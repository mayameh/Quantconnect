...
class MayankAlgo_Production(QCAlgorithm):
    def initialize(self) -> None:
        ...
        # ✅ FIX: Increased warmup to ensure all indicators ready (300 hours = ~37 trading days)
        ...
        try:
            ...
            for key in ["ema_50", "rsi", "macd"]:
                ...
                try:
                    ...
                    # ✅ FIX: Try manual history-based warmup as fallback for problematic symbols like META
                    try:
                        periods = 100 if key == "ema_50" else 50
                        history = self.history(symbol, periods, resolution)
                        if not history.empty and 'close' in history.columns:
                            for index, row in history.iterrows():
                                indicator.update(index[1], row['close'])
                            self.logger.info(f"Manual warmup successful for {symbol.value} ({key})")
                        else:
                            self.logger.warning(f"No history available for {symbol.value} ({key})")
                    except Exception as fallback_error:
                        self.logger.warning(f"Both warmup methods failed for {symbol.value} ({key}): {fallback_error}")
        except Exception as e:
    ...
    def _evaluate_signals(self) -> None:
        ...
        for symbol in algo_invested:  # Changed from all_invested to algo_invested
            try:
                ...
                elif pnl_pct >= 0.08:  # ✅ FIX: Increased from 8% to 12% to let winners run
                    self.debug(f"TAKE PROFIT TRIGGERED: {symbol.value}")
                    should_exit, reason = True, "take_profit"
                elif held_time >= timedelta(hours=30) and pnl_pct >= 0.018:  # ✅ FIX: Hold longer for better profits
                    should_exit, reason = True, "profit_lock"
                elif held_time >= timedelta(days=14):  # ✅ FIX: Reduced from 14 to 10 days to recycle capital faster
        ...
        not_ready_symbols = []
        not_ready_details = {}  # ✅ FIX: Track which indicators aren't ready
        for symbol in self._all_symbols:
            try:
                ...
                if not self._is_symbol_ready(symbol):
                    ...
                    # ✅ FIX: Detailed diagnosis for persistent issues
                    indicators = self._indicators.get(symbol, {})
                    not_ready_details[symbol.value] = {
                        'macd': indicators.get('macd').is_ready if indicators.get('macd') else False,
                        'rsi': indicators.get('rsi').is_ready if indicators.get('rsi') else False,
                        'ema': indicators.get('ema_50').is_ready if indicators.get('ema_50') else False
                    }
                    continue
                indicators = self._indicators.get(symbol)
                if not indicators:
        ...
        try:
            ...
            for symbol in list(self._algo_managed_positions):
                ...
                try:
                    ...
                    # ✅ FIX: Apply same exit conditions as in _evaluate_signals (updated thresholds)
                    if pnl_pct <= -0.03:
                        should_exit, reason = True, "stop_loss"
                    elif pnl_pct >= 0.08:
                        should_exit, reason = True, "take_profit"
                    ...
                    if should_exit:
#                        self.liquidate(symbol, tag=f"end_of_algo_{reason}")
